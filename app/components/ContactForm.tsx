...
export function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!scope || scope.trim().length < 10)
      newErrors.scope =
        "Please briefly describe your requirements (min 10 chars).";
  ...
  if (isSuccess) {
    return (
      <div
        className="flex flex-col items-center justify-center gap-6 bg-surface/10 p-6 md:p-10 border border-white/10 backdrop-blur-sm h-full text-center"
        aria-live="polite"
      >
        ...
        <p className="text-muted text-sm max-w-xs leading-relaxed">
          Your requirements have been logged. Our engineering leads will review
          your brief and coordinate next steps.
        </p>
  ...
  return (
    <form
      ...
      noValidate
      className="flex flex-col gap-8 bg-surface/10 p-6 md:p-10 border border-white/10 backdrop-blur-sm h-full relative overflow-hidden"
    >
      {/* Loading overlay */}
      {isSubmitting && (
        <div className="absolute inset-0 z-10 bg-base/80 backdrop-blur-md flex flex-col items-center justify-center border border-accent/20">
          <div className="flex gap-1 mb-6">
            <div
              className="h-2 w-2 bg-accent animate-pulse"
      ...
      <div className="flex flex-col gap-3 group">
        <div className="flex justify-between items-end">
          <label
            className="font-mono text-[10px] text-muted uppercase tracking-widest group-focus-within:text-ink transition-colors"
            htmlFor="name"
          >
            NAME / ORGANIZATION
          </label>
          {errors.name && (
            <span className="text-[10px] text-red-400 font-mono" role="alert">
              {errors.name}
            </span>
          )}
        </div>
